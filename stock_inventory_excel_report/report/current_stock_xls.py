...
class StockReportXls(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.write(9, 2, 'Brand', format3)
        sheet.write(9, 3, 'Internal Reference', format3)
        sheet.write(9, 4, 'Beginning', format3)
        sheet.write(9, 5, 'Purchase Qty', format3)
        sheet.write(9, 6, 'Purchase Return', format3)
        sheet.write(9, 7, 'Sale Qty', format3)
        sheet.write(9, 8, 'Sale Return', format3)
        sheet.write(9, 9, 'POS qty', format3)
        sheet.write(9, 10, 'POS Return', format3)
        sheet.write(9, 11, 'Internal Transfer', format3)
        sheet.write(9, 12, 'Adjustment ', format3)
        sheet.write(9, 13, 'Scrap ', format3)
        sheet.write(9, 14, 'Closing Qty ', format3)
        ...
        if data.get('form', {}).get('sort_order', '') != 'product_category':
            ...
            for warehouse in lines.items():
                ...
                for location in warehouse[1]:
                    ...
                    for l in warehouse[1][location]:
                        ...
                        sheet.write(y_offset, 4, begin_qty, format5_1)
                        sheet.write(y_offset, 5, l['product_qty_in'], format5_1)
                        sheet.write(y_offset, 6, l['product_qty_in_return'], format5_1)
                        sheet.write(y_offset, 7, l['product_qty_out'], format5_1)
                        sheet.write(y_offset, 8, l['product_qty_out_return'], format5_1)
                        sheet.write(y_offset, 9, l['pos_qty'], format5_1)
                        sheet.write(y_offset, 10, l['pos_return_qty'], format5_1)
                        sheet.write(y_offset, 11, l['product_qty_internal'], format5_1)
                        sheet.write(y_offset, 12, l['product_qty_adjustment'], format5_1)
                        sheet.write(y_offset, 13, l['product_qty_scrap'], format5_1)
                        ...
                        sheet.write(y_offset, 14, end_qty, format5_1)
                    ...
                    sheet.write(y_offset, 3, 'Total', format5)
                    sheet.write(y_offset, 4, total_begin, format5_border_top)
                    sheet.write(y_offset, 5, total_in, format5_border_top)
                    sheet.write(y_offset, 6, total_in_return, format5_border_top)
                    sheet.write(y_offset, 7, total_out, format5_border_top)
                    sheet.write(y_offset, 8, total_out_return, format5_border_top)
                    sheet.write(y_offset, 9, total_pos, format5_border_top)
                    sheet.write(y_offset, 10, total_pos_return, format5_border_top)
                    sheet.write(y_offset, 11, total_int, format5_border_top)
                    sheet.write(y_offset, 12, total_adj, format5_border_top)
                    sheet.write(y_offset, 13, total_scrap, format5_border_top)
                    sheet.write(y_offset, 14, total_end_qty, format5_border_top)
                    y_offset += 2
            ...
            sheet.write(y_offset, 1, "", format5)
            sheet.write(y_offset, 2, "", format5)
            sheet.write(y_offset, 3, "Total Inventory", format5)
            sheet.write(y_offset, 4, grand_total_begin, format5)
            sheet.write(y_offset, 5, grand_total_in, format5)
            sheet.write(y_offset, 6, grand_total_in_return, format5)
            sheet.write(y_offset, 7, grand_total_out, format5)
            sheet.write(y_offset, 8, grand_total_out_return, format5)
            sheet.write(y_offset, 9, grand_total_pos, format5)
            sheet.write(y_offset, 10, grand_total_pos_return, format5)
            sheet.write(y_offset, 11, grand_total_int, format5)
            sheet.write(y_offset, 12, grand_total_adj, format5)
            sheet.write(y_offset, 13, grand_total_scrap, format5)
            sheet.write(y_offset, 14, grand_total_end_qty, format5)
            y_offset += 1
        else:
