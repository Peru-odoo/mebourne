...
class SaleReportCustomer(models.TransientModel):
    ...
    def action_print_sale_customer(self):
        ...
        for wizard in self:
            ...
            customer_stat = """SELECT distinct partner_id FROM sale_order where invoice_status='invoiced'""";
            self.env.cr.execute(customer_stat)
            ...
            for res in result:
                ...
                while (next_month_start_date <= end_date):
                    query = """SELECT sum(total) total,sum(qty) qty
                                ...
                                WHERE a.invoice_status = 'invoiced' AND a.partner_id = '""" + str(partner.id) + """' 
                                AND a.id  = c.order_id
