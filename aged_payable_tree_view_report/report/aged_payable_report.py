    ...
    def _query(self, with_clause='', fields={}, groupby='', from_clause='', where=''):
        ...
        select_ = """
            ...
            case when (DATE_PART('day', CURRENT_TIMESTAMP::timestamp - am.invoice_date_due::timestamp)) < 0
                then concat(round(am.amount_residual / (SELECT currency_rate.rate 
            ...
            case when (DATE_PART('day', CURRENT_TIMESTAMP::timestamp - am.invoice_date_due::timestamp) >= 0) and (DATE_PART('day', CURRENT_TIMESTAMP::timestamp - am.invoice_date_due::timestamp) <= 10)
                then concat(round(am.amount_residual / (SELECT currency_rate.rate 
                                            FROM res_currency_rate currency_rate
                                            JOIN res_currency currency ON (currency_rate.currency_id = currency.id)
                                            WHERE currency.name = cur.name
            ...
            case when cur.name ='USD'
                then concat('$',am.amount_residual)
                else '$0.0'
            ...
            case when cur.name = 'EUR'
                then concat('EUR',am.amount_residual)
                else 'EUR 0.0' 
                end as amount_due_euro,
            case when cur.name = 'MMK'
                then CONCAT( CAST(am.amount_residual AS TEXT),'K')
                else concat(round(am.amount_residual / (SELECT currency_rate.rate 
                                            FROM res_currency_rate currency_rate
                                            JOIN res_currency currency ON (currency_rate.currency_id = currency.id)
                                            WHERE currency.name = cur.name
                                            ORDER BY currency_rate.create_date DESC LIMIT 1),2),'K'
        ...
        where_ = """
            am.invoice_payment_state = 'not_paid' and
            am.type = 'in_invoice'
            %s
        """ % where
